# -*- coding: utf-8 -*-
"""JsonData base-class; models Spark JSON objects as native Python objects.
The JsonData class models any JSON object passed to it as a string or Python
dictionary as a native Python object; providing attribute access access using
native object.attribute syntax.
JsonData is intended to serve as a base-class, which provides inheritable
functionality, for concrete sub-classes that model specific Cisco Spark data
objects (rooms, messages, webhooks, etc.).  The JsonData base-class provides
...
class JsonData(object):
    """Model JSON objects as native Python objects."""

    def __init__(self, json):
        """Init a new JsonData object from a JSON dictionary or string.
        Args:
            json(dict, str): Input JSON object.
        Raises:
            TypeError: If the input object is not a dictionary or string.
    ...
    def __getattr__(self, item):
        ...
        JsonData.__getattr__() checks the original JSON object to see if the
